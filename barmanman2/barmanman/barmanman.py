...
table = open("data/records.txt", mode="rt").readlines()
record_table_1 = []
...
def load_image(name, colorkey=None):
    fullname = os.path.join('../../Desktop/barmanman/data', name)
...
while gl_running:
    if game == 0:
        ...
        sink_group = pygame.sprite.Group()


        class Barmaman(pygame.sprite.Sprite):
        ...
        class Record(pygame.sprite.Sprite):
            ...
            def update(self):
                name_day_coins = main_font.render('Имя      День Заработано Имя      День Заработано',
                                                  True, (255, 255, 255))
        ...
        class Bettor(pygame.sprite.Sprite):
                ...
                if event.type == pygame.KEYDOWN:
                    if bettor.print:
                        if event.key == pygame.K_BACKSPACE:
                            bettor.red_text(-1)
                        elif event.key != pygame.K_KP_ENTER and event.key != pygame.K_RETURN:
                ...
                if x in range(tiles.rect.x, tiles.rect.x + cell) and y in range(tiles.rect.y,
                    ...
                    if tiles.get_void() and pygame.sprite.spritecollideany(tiles, blockers_group):
                        if (tp == 'rack' and coins.my_money() >= 20) or (
                                tp == 'case' and coins.my_money() >= 10) or (
                                tp == 'sink' and coins.my_money() >= 50) or (
                                tp == 'fillmachin' and coins.my_money() >= 75):
                ...
                if x in range(tiles.rect.x, tiles.rect.x + cell) and y in range(tiles.rect.y,
                    ...
                    if tiles.get_void() and pygame.sprite.spritecollideany(tiles, blockers_group):
                        if tp == 'rack' and coins.my_money() >= 20:
                            ...
                            coins.change_cap(-10)
                        elif tp == 'sink' and coins.my_money() >= 50:
                            coins.change_cap(-50)
                        elif tp == 'fillmachin' and coins.my_money() >= 75:
                            coins.change_cap(-75)
                        tiles.void = False
                        blockers_group.update(player.rect.x, player.rect.y)
                        pygame.sprite.spritecollideany(tiles, blockers_group).select()
        ...
        cursor_img = load_image('dream.png', -1)
        cursor_img = pygame.transform.scale(cursor_img, (20, 36))
        ...
        sink = load_image('sink.jpg')
        sink = pygame.transform.scale(sink, (90, 90))

        fillingfachine = load_image('fillmachins.png')
        fillingmachine = pygame.transform.scale(fillingfachine, (90, 90))

        em = pygame.transform.scale(fillingfachine, (0, 0))
        ...
        around_group = pygame.sprite.Group()
        ...
        fillingmachine_group = pygame.sprite.Group()
        ...
        class Case(pygame.sprite.Sprite):  # Хранилище ингредиентов
            ...
            def store(self, font):
                ...
                if pos[0] in range(211, 416) and pos[1] in range(940, 965):
                    ...
                    cherry_compote = font.render('Вишневый компот', True, (255, 0, 0))
                ...
                if pos[0] in range(446, 575) and pos[1] in range(940, 965):
                    ...
                    soda = font.render('Газировка', True, (94, 210, 184))
        ...
        class Sink(pygame.sprite.Sprite):
            def __init__(self, pos):
                pos_x = pos[0]
                pos_y = pos[1]
                super().__init__(all_sprites, sink_group, obj_group)
                self.image = sink
                self.rect = self.image.get_rect().move(
                    tile_width * pos_x + 5, tile_height * pos_y + 5)
                self.wall = True
            ...
            def get_info(self, font, store_font):
                viw = font.render('Раковина', True, (255, 255, 255))
                screen.blit(viw, (40, 920))


        class FillingMachine(pygame.sprite.Sprite):
            def __init__(self, pos):
                pos_x = pos[0]
                pos_y = pos[1]
                super().__init__(all_sprites, fillingmachine_group, obj_group)
                self.image = fillingmachine
                self.rect = self.image.get_rect().move(
                    tile_width * pos_x + 5, tile_height * pos_y + 5)
                self.wall = True
                self.interaction = pygame.Rect(tile_width * pos_x - cell // 2, tile_height * pos_y - cell // 2,
                                               2 * cell, 2 * cell)
                self.zone = Around(self.interaction)
                self.gl = []

            def around(self):
                return list(pygame.sprite.spritecollide(self.zone, drinks_group, False))
            ...
            def get_info(self, font, store_font):
                viw = font.render('Разливочный механизм', True, (255, 255, 255))
                screen.blit(viw, (40, 920))
                for i, st in enumerate(self.gl):
                    nap = font.render(st[0] + ' ' + str(st[1]), True, (255, 255, 255))
                    screen.blit(nap, (40, 940 + 20 * i))

            def take(self):
                bochki = []
                if self.gl and 1 - sum([i[1] for i in glass.gl]) >= sum([i[1] for i in self.gl]):
                    for j in self.gl:
                        for i in self.around():
                            if j[0] == i.sort and j[1] <= i.fullness:
                                bochki.append((i, j[1]))
                    if len(bochki) == len(self.gl):
                        for i in bochki:
                            i[0].fillmachines(i[1])
                        if self.gl[0][0] == glass.gl[-1][0]:
                            glass.gl[-1][1] = glass.gl[-1][1] + self.gl[0][1]
                            glass.gl += self.gl[1:]
                        else:
                            glass.gl += self.gl
        ...
        class Around(pygame.sprite.Sprite):
            def __init__(self, rect):
                super().__init__(all_sprites, around_group)
                self.image = em
                self.rect = rect


        class Visitor(pygame.sprite.Sprite):
        ...
        class Cocktail(pygame.sprite.Sprite):
        ...
        class Inside(pygame.sprite.Sprite):
        ...
        class Days:
            ...
            def update(self, dt):
                ...
                if self.m == 5:
                    self.d += 1
                    coins.change_cap(-1 * self.d * 5 - num_of_people * 10)
        ...
        while running:
            cursor_img_rect.center = pygame.mouse.get_pos()  # позиция курсора
            blocker = None
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_UP or event.key == pygame.K_w:
                        ...
                        sell(blocker)
                    elif event.key == pygame.K_f and pygame.sprite.spritecollideany(blocker,
                                                                                    fillingmachine_group):  # Взаимодействие с наполнителем
                        pygame.sprite.spritecollide(blocker, fillingmachine_group, False)[0].remember()
                    elif event.key == pygame.K_e and pygame.sprite.spritecollideany(blocker, fillingmachine_group):
                        if pygame.sprite.spritecollide(blocker, fillingmachine_group, False)[0].gl:
                            pygame.sprite.spritecollide(blocker, fillingmachine_group, False)[0].take()
                        ...
                        while esc:
                            ...
                            pygame.display.flip()
                        pygame.mouse.set_visible(False)
                    elif event.key == pygame.K_1 and get_place(pygame.mouse.get_pos(),
                                                               'sink'):
                        a = get_place(pygame.mouse.get_pos(), '')
                        Sink(place(pygame.mouse.get_pos(), 'sink'))
                    elif event.key == pygame.K_2 and get_place(pygame.mouse.get_pos(),
                                                               'fillmachin'):
                        FillingMachine(place(pygame.mouse.get_pos(), 'fillmachin'))
                    if not running:
                        break
                    if not gl_running:
                        pygame.quit()
                ...
                if keys[pygame.K_f] and pygame.sprite.spritecollideany(blocker, sink_group):  # Сливание жидкости
                    glass.pour_out()
                    glass.update()
                if keys[pygame.K_SPACE] and glass.volume < 1:  # Набор жидкости
                    if pygame.sprite.spritecollideany(blocker, case_group):
            ...
            screen.blit(cursor_img, cursor_img_rect)  # Отрисовка курсора
        ...
        if not gl_running:
            pygame.quit()
if all_coins > 0:
    records = open("../../Desktop/barmanman/data/records.txt", mode="rt").readlines()
    if records:
        record = []
        for i in records:
            if i == '\n':
    ...
    records = open("../../Desktop/barmanman/data/records.txt", mode="w")
